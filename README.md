...
Objetivo do teste
    ...
    Criar um projeto django com um app de cotação de fretes (Delivery);
    Criar um modelo de pedidos (Order) com os campos: number, amount, weight, width, height, length, zip_from, zip_to;
    Criar uma tela no admin para criar, e editar os pedidos;
    Criar um modelo de cotações de fretes (Freight) com os campos: order (FK), carrier, delivery_time, delivery_cost, external_freight_id;
    Criar uma action no modelo de Order para fazer o cálculo do frete via API (ver instruções abaixo) e gravar os resultados no modelo de Freight;
    Colocar as entradas de Freight como inline no django admin do Order;
